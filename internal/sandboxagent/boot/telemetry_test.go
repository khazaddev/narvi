...
func TestRunHooks_RecordsHookRerunDurationMetric(t *testing.T) {
	...
	if err := boot.RunHooks(ctx, sup, workspaceDir, repos, sandboxboot.BootModeFresh, nil, nil, nil,
...
func TestRunHooks_RecordsHookRerunDurationMetric_CarriesBootModeAndWorkspaceMovedAttributes(t *testing.T) {
	...
	if err := boot.RunHooks(ctx, sup, workspaceDir, repos, sandboxboot.BootModeRepoImage, workspaceMoved, nil, nil,
		5*time.Second, time.Second, time.Millisecond); err != nil {
		t.Fatalf("RunHooks() error = %v, want nil", err)
	}
