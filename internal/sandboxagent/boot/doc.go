...
// whenever a repo has no .narvi/services.yml. Step 15/16/17 are expected to
// extend this package (or add sibling ones) further as the boot sequence
// grows; this Step only covers what it actually has inputs for -- notably,
// RunBoot (like RunHooks before it) takes its repo list as a plain
// []RepoInfo parameter, and main.go wires it with an empty slice today (a
// real, intentional no-op until Step 15 populates it from SESSION_CONFIG).
//
