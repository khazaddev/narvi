...
// Step 43 (§19.6): end-to-end tests of the graduated setup-rerun ladder,
...
func TestSetupRerunLadder_LogsStructuredDecisionsForEachTier(t *testing.T) {
	...
	if err := boot.RunBoot(context.Background(), sup, workspaceDir, repos, sandboxboot.BootModeRepoImage, workspaceMoved, ladder,
		noopReporter, 10*time.Second, time.Second, testReadinessTimeout, testReadinessPollInterval); err != nil {
		t.Fatalf("RunBoot() error = %v, want nil", err)
	}
