...
// embedded migration up, and returns a ready *pgxpool.Pool -- this
// package's own copy of internal/app/egressmode's own newTestPool (this
// codebase's established per-package precedent: "each DB-touching
// package builds its own copy ... rather than sharing one across package
// boundaries"). One fresh container per test, not a shared one: this
// package's own integration suite is small enough that the outboxworker/
// httpapi shared-container-plus-TRUNCATE optimization is not worth its
// own added complexity here.
