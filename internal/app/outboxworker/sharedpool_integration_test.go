...
// This package's integration suite has ~23 test functions across 4
// *_integration_test.go files (all package outboxworker_test -- no
// white-box split needed here), and before this file, every one of those
// spun up its OWN brand-new throwaway Postgres container via
// testcontainers-go (builder_integration_test.go's own newTestPool,
// reused by all 4 files). Even at a healthy ~1-3s per container start,
// that is real container-churn overhead across every test run, before
// any actual test logic runs.
//
...
//
// builder_integration_test.go already had its own TestMain (wiring a
// single global OTel MeterProvider for this whole binary's metric-delta
// assertions, e.g. readDeadLetterCount) -- that TestMain now lives here,
// merged with this file's own container startup, since Go allows exactly
// one TestMain per test binary. otelReader itself stays declared in
// builder_integration_test.go (its only reader is there too).
//
// # Why this is safe against this package's own async Actor background work
//
// Most of this package (Builder.PumpOnce/RefreshOnce-equivalent logic)
// never touches sessionactor.Registry at all -- it calls ports.Notifier.
// Deliver directly. The one exception is sentinelautofix_integration_
// test.go: sentinelAutoFixNotifier.Deliver calls httpapi.SpawnChildSession,
// which calls httpapi.TriggerDispatch -- the identical fire-and-forget
// async-actor-dispatch pattern httpapi's own tests rely on. In all 3
// tests there that construct a Registry, `pool := newTestPool(t)` runs
// strictly BEFORE `sessionactor.NewRegistry(...)` + `t.Cleanup(func() {
// _ = registry.Shutdown() })` -- the same pool-before-registry ordering
// httpapi's own doc comment explains, which combined with Go's LIFO
// t.Cleanup semantics guarantees registry.Shutdown() (a synchronous,
// fully-draining wait) always finishes before this file's own
// truncate-and-restore cleanup ever runs. This package also never calls
// t.Parallel() anywhere (verified); its two "Concurrent"-named tests
// (TestPumpOnce_ConcurrentTicksNeverDoubleClaim, TestPumpOnce_
// SlowSequentialDelivery_ConcurrentTickNeverStealsRowMidDelivery) fully
// join every goroutine they spawn (sync.WaitGroup.Wait/errgroup.Wait)
// before the test function itself returns, so no leftover goroutine or
// advisory lock survives into the next test's setup.
//
...
// (none touch an unscoped count(*)) -- reproducing exactly what a fresh
// container already guaranteed (a byte-for-byte-empty, freshly-migrated
// database at the start of every test) via TRUNCATE avoids having to
// keep re-auditing every future test added to this package for that
// same risk. prepareDatabaseReset (below) also restores any
// migration-seeded row (e.g. prompt_templates' one seed row,
// migrations/000033_intent_classifier.up.sql) generically, so this keeps
...
// MeterProvider builder_integration_test.go's own otelReader needs (that
// file used to own this TestMain itself, before this package also
// needed a shared container -- see its own doc comment).
func TestMain(m *testing.M) {
	ctx := context.Background()

	container, connStr, err := startSharedTestContainer(ctx)
	if err != nil {
		log.Fatalf("outboxworker: start shared integration-test container: %v", err)
	}

	if err := runMigrations(connStr); err != nil {
		log.Fatalf("outboxworker: run migrations against shared integration-test container: %v", err)
	}

	pool, err := narvipg.NewPool(ctx, connStr)
	if err != nil {
		log.Fatalf("outboxworker: open shared integration-test pool: %v", err)
	}

	if err := prepareDatabaseReset(ctx, pool); err != nil {
		log.Fatalf("outboxworker: prepare shared integration-test database reset: %v", err)
	}
	...
	if err := testcontainers.TerminateContainer(container); err != nil {
		log.Printf("outboxworker: terminate shared integration-test container: %v", err)
	}
	os.Exit(code)
}
...
// this package's own (limited) async sessionactor.Actor background work.
...
func IntegrationTestPoolAndConnStr(t *testing.T) (*pgxpool.Pool, string) {
	t.Helper()
	if sharedPool == nil {
		t.Fatal("outboxworker: IntegrationTestPoolAndConnStr called before TestMain finished setting up the shared pool")
	}
...
func resetSharedTestDatabase(t *testing.T) {
	...
	if _, err := sharedPool.Exec(ctx, truncateStatement); err != nil {
		t.Fatalf("outboxworker: reset shared integration-test database (truncate): %v", err)
	}
	for _, stmt := range restoreStatements {
		if _, err := sharedPool.Exec(ctx, stmt); err != nil {
			t.Fatalf("outboxworker: reset shared integration-test database (restore seed data): %v", err)
		}
	}
}
