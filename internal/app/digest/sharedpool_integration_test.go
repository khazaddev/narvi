...
func TestMain(m *testing.M) {
	...
	if err != nil {
		log.Fatalf("digest: start shared integration-test container: %v", err)
	}

	if err := runMigrations(connStr); err != nil {
		log.Fatalf("digest: run migrations against shared integration-test container: %v", err)
	}

	pool, err := narvipg.NewPool(ctx, connStr)
	if err != nil {
		log.Fatalf("digest: open shared integration-test pool: %v", err)
	}

	if err := prepareDatabaseReset(ctx, pool); err != nil {
		log.Fatalf("digest: prepare shared integration-test database reset: %v", err)
	}
	...
	if err := testcontainers.TerminateContainer(container); err != nil {
		log.Printf("digest: terminate shared integration-test container: %v", err)
	}
	os.Exit(code)
}
...
func IntegrationTestPool(t *testing.T) *pgxpool.Pool {
	t.Helper()
	if sharedPool == nil {
		t.Fatal("digest: IntegrationTestPool called before TestMain finished setting up the shared pool")
	}
...
func resetSharedTestDatabase(t *testing.T) {
	...
	if _, err := sharedPool.Exec(ctx, truncateStatement); err != nil {
		t.Fatalf("digest: reset shared integration-test database (truncate): %v", err)
	}
	for _, stmt := range restoreStatements {
		if _, err := sharedPool.Exec(ctx, stmt); err != nil {
			t.Fatalf("digest: reset shared integration-test database (restore seed data): %v", err)
		}
	}
}
