...
type fakeDecisionInboxSourceControl struct {
	...
	// openPRsTruncated/openPRsErr (Step 60 review finding C1, TEST BATCH: this
	...
	// receives, in order -- Step 60 review finding B3 (TEST BATCH): "reverting
...
// scoping actually excludes/includes the right rows (Step 60 review finding
...
// with no prior coverage (Step 60 review finding T4/T6):
...
//     domain Item itself -- the read-model half of Step 60 review finding
...
func TestBuild_PRLabelVariations(t *testing.T) {
	...
	if !handoffPR.CIGreen {
		t.Error("PR #21 (handoff) CIGreen = false, want true -- PR-shaped fields must still populate for a handoff row (Step 60 review finding C4's read-path half)")
	}
...
// regression test for Step 60 review finding P1-4 (second round): before this
...
func TestBuild_HasChangesRequestedDemotesFromReadyToMerge(t *testing.T) {
	...
	if !pr40.HasChangesRequested {
		t.Error("PR #40 HasChangesRequested = false, want true -- the domain Item field itself must also surface this fact (Step 60 review finding P1-4)")
	}
}
...
func TestBuild_CodeOwnersResolvedAgainstBaseRefNeverHead(t *testing.T) {
	...
	for _, call := range fakeSCM.codeOwnersCalls {
		if call.Ref != wantBaseRef {
			t.Errorf("ResolveCodeOwners called with Ref = %q, want the PR's own BASE ref %q (never HeadSHA -- Step 60 review finding B3)", call.Ref, wantBaseRef)
		}
	}
}

// TestBuild_SCMFetchFailedSignal proves Result.SCMFetchFailed actually
// becomes true for its own producers (Step 60 review findings P1-2/P1-3,
// second round; Step 60 review finding C1, TEST BATCH: SCMFetchFailed=true
...
func TestBuild_SCMFetchFailedSignal(t *testing.T) {
	...
	t.Run("TruncatedRead_StillSurfacesPartialItemsWithARealAsOf", func(t *testing.T) {
		...
		if result.SCMAsOf == nil {
			t.Error("SCMAsOf = nil, want non-nil -- a truncated read is still a REAL, if partial, fetch: SCMAsOf and SCMFetchFailed are no longer mutually exclusive (Step 60 review finding P1-2)")
		}
	})
...
func TestBuild_SentinelFixStoreErrorDegradesTheReadButNeverPanics(t *testing.T) {
	...
	if !result.SCMFetchFailed {
		t.Error("SCMFetchFailed = false, want true -- a per-PR SentinelFixStore error must degrade the overall read (Step 60 review finding P1-3), never silently render a fresh, complete queue with that row simply missing")
	}
}
...
func TestBuild_CredentialResolutionErrorDegradesRatherThanRenderingNoGitHub(t *testing.T) {
	...
	if !result.SCMFetchFailed {
		t.Error("SCMFetchFailed = false, want true -- a genuine identity-store error must never render identically to \"no GitHub linked at all\" (Step 60 review finding P2-1)")
	}
}
...
func TestBuild_ReviewDecisionDegraded_DemotesFromReadyToMerge(t *testing.T) {
	...
	if !result.SCMFetchFailed {
		t.Error("SCMFetchFailed = false, want true -- a per-PR degraded review-decision read must mark the overall read incomplete (Step 62 review finding C4)")
	}
}
...
func TestBuild_Contested_HasChangesRequestedHalf_RecordsOverridden(t *testing.T) {
	...
	if total != 1 || contested != 1 {
		t.Errorf("outcome counts = (total=%d, contested=%d), want (1, 1) -- the engine would have approved this PR on every real criterion, but a reviewer requested changes: this MUST record 'overridden' (Step 62 review finding T1 -- this exact half was previously unreachable)", total, contested)
	}
}
