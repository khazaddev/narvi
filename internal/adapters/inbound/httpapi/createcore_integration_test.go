...
// foremost, with each provider's own webhook ingress package (Steps
// 32-34's GitHub/Slack/Linear adapters, internal/adapters/inbound/linear
// among them) as their only outside callers. This file builds its own
// minimal testcontainers-Postgres rig
...
func TestCreateSessionOnTx_CallerRollback_PersistsNothing(t *testing.T) {
	...
	created, hasPrompt, cerr := CreateSessionOnTx(ctx, tx, sessions, turns, environments, req, nilCreator)
	if cerr != nil {
...
func TestCreateSessionOnTx_ValidationFailure_NeverTouchesTx(t *testing.T) {
	...
	_, hasPrompt, cerr := CreateSessionOnTx(ctx, tx, sessions, turns, environments, req, nilCreator)
	if cerr == nil {
