...
// scoped to the REAL authenticated caller (platform.UserFromContext, via
// authenticatedUserID in helpers.go) with a platform.Timeouts.WSTokenTTL
// expiry (§6.2: "24h TTL"), and responds 200 with restdtos.WSTokenResponse
// carrying the PLAINTEXT token -- the only time it is ever returned,
// matching §6.2's "hashed at rest" requirement literally.
