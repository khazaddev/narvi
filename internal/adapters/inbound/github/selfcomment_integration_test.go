...
// addition) for the full "why", including the documented, known
// limitation this test file's own scope deliberately stays inside (a
// GitHub App installation's own "[bot]"-suffixed login is NOT covered --
// see that filter's own doc comment for why closing that fully is out of
// this batch's scope).
package github_test
...
// "PAT-style bot account" case the filter's own doc comment describes as
// the one it DOES correctly catch) is never treated as a mention, even
// though its own body genuinely satisfies compileMentionPattern -- no
...
func TestGitHubIntegration_SelfComment_MatchingBotHandle_Ignored(t *testing.T) {
	...
	// one case this filter is scoped to actually catch (see handler.go's
	// own doc comment on the filter for the broader, deliberately-NOT
	// -solved GitHub-App "[bot]"-suffix case).
