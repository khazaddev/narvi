...
// This package's integration suite has ~49 test functions across 11
// *_integration_test.go files (all package slack_test -- no white-box
// split needed here, unlike internal/adapters/inbound/httpapi/internal/
// app/sessionactor), and before this file, every one of those spun up
// its OWN brand-new throwaway Postgres container via testcontainers-go
// (handler_integration_test.go's own newTestPool), ran every embedded
// migration, opened a pool, and tore the whole thing down again. Even at
// a healthy ~1-3s per container start, that is 49-147s of pure
// container-churn overhead alone, before any actual test logic runs.
//
// This mirrors, package-for-package, internal/adapters/inbound/httpapi's
// own perf/httpapi-integration-container-reuse fix and internal/app/
// sessionactor's own identical follow-up (see either package's own
...
// slackTestRig/newSlackTestRig and this file's sibling test files' own
// small rig variants) exactly like httpapi's own tests do: every call
// site obtains its pool (IntegrationTestPool, below -- which registers
// this file's own truncate-and-restore cleanup) strictly BEFORE
// constructing a Registry (which registers its OWN t.Cleanup(func() {
...
// cleanup ever runs -- verified directly across every NewRegistry call
// site in this package, including the real-goroutine-race test in
// concurrency_integration_test.go. This package also never calls
// t.Parallel() anywhere (verified).
//
// # Why a full TRUNCATE, not per-test unique IDs alone
//
// Same rationale as httpapi's own version: an audit of this package's
// own tests found genuine whole-table/unscoped assertions that would
// break under any leftover cross-test state -- e.g.
// handler_integration_test.go's own `SELECT count(*) FROM sessions`
// (TestHandler_URLVerification_DoesNotProcessAsEvent, expecting exactly
// 0), three more in identity_integration_test.go, and turn_integration_
// test.go's own `SELECT count(*) FROM audit_log WHERE action=
// 'turn.create'` (unscoped by resource). Reproducing exactly what a
// fresh container already guaranteed (a byte-for-byte-empty,
// freshly-migrated database at the start of every test) via TRUNCATE was
// judged the safer, smaller change here too, matching httpapi's own
// documented tradeoff exactly. prepareDatabaseReset (below) also
// restores any migration-seeded row (e.g. prompt_templates' one seed
// row, migrations/000033_intent_classifier.up.sql) generically, so this
// keeps working unchanged even if a future migration seeds a different/
// additional table.
package slack_test
...
// concern here, unlike internal/app/sessionactor's own version of this
// same file.
func TestMain(m *testing.M) {
	...
	if err != nil {
		log.Fatalf("slack: start shared integration-test container: %v", err)
	}

	if err := runMigrations(connStr); err != nil {
		log.Fatalf("slack: run migrations against shared integration-test container: %v", err)
	}

	pool, err := narvipg.NewPool(ctx, connStr)
	if err != nil {
		log.Fatalf("slack: open shared integration-test pool: %v", err)
	}

	if err := prepareDatabaseReset(ctx, pool); err != nil {
		log.Fatalf("slack: prepare shared integration-test database reset: %v", err)
	}
	...
	if err := testcontainers.TerminateContainer(container); err != nil {
		log.Printf("slack: terminate shared integration-test container: %v", err)
	}
	os.Exit(code)
}
...
func IntegrationTestPoolAndConnStr(t *testing.T) (*pgxpool.Pool, string) {
	t.Helper()
	if sharedPool == nil {
		t.Fatal("slack: IntegrationTestPoolAndConnStr called before TestMain finished setting up the shared pool")
	}
...
func resetSharedTestDatabase(t *testing.T) {
	...
	if _, err := sharedPool.Exec(ctx, truncateStatement); err != nil {
		t.Fatalf("slack: reset shared integration-test database (truncate): %v", err)
	}
	for _, stmt := range restoreStatements {
		if _, err := sharedPool.Exec(ctx, stmt); err != nil {
			t.Fatalf("slack: reset shared integration-test database (restore seed data): %v", err)
		}
	}
}
