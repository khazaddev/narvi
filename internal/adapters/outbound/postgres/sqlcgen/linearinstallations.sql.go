// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.31.1
// source: linearinstallations.sql

package sqlcgen

import (
	"context"

	"github.com/jackc/pgx/v5/pgtype"
)

const getLinearInstallationByOrganizationID = `-- name: GetLinearInstallationByOrganizationID :one
SELECT organization_id, app_user_id, access_token_encrypted, refresh_token_encrypted, expires_at, connected_by_user_id, created_at, updated_at FROM linear_installations
WHERE organization_id = $1
`

// The outbound AgentActivity call's own lookup: which access token (if
// any) can post back to this Linear workspace? A pgx.ErrNoRows result
// means no admin has connected this workspace yet -- the caller skips the
// outbound call entirely rather than failing the whole webhook.
func (q *Queries) GetLinearInstallationByOrganizationID(ctx context.Context, organizationID string) (LinearInstallation, error) {
	row := q.db.QueryRow(ctx, getLinearInstallationByOrganizationID, organizationID)
	var i LinearInstallation
	err := row.Scan(
		&i.OrganizationID,
		&i.AppUserID,
		&i.AccessTokenEncrypted,
		&i.RefreshTokenEncrypted,
		&i.ExpiresAt,
		&i.ConnectedByUserID,
		&i.CreatedAt,
		&i.UpdatedAt,
	)
	return i, err
}

const updateLinearInstallationToken = `-- name: UpdateLinearInstallationToken :one
UPDATE linear_installations
SET access_token_encrypted  = $2,
    refresh_token_encrypted = $3,
    expires_at              = $4,
    updated_at               = now()
WHERE organization_id = $1
RETURNING organization_id, app_user_id, access_token_encrypted, refresh_token_encrypted, expires_at, connected_by_user_id, created_at, updated_at
`

type UpdateLinearInstallationTokenParams struct {
	OrganizationID        string             `json:"organization_id"`
	AccessTokenEncrypted  []byte             `json:"access_token_encrypted"`
	RefreshTokenEncrypted []byte             `json:"refresh_token_encrypted"`
	ExpiresAt             pgtype.Timestamptz `json:"expires_at"`
}

// Refreshes a stored token pair after a real refresh_token exchange
// (Linear's own access tokens are short-lived, §-confirmed during this
// Step's investigation: "valid for 24 hours"), WITHOUT touching
// app_user_id/connected_by_user_id -- a refresh never changes which
// workspace or which app-user this installation is.
func (q *Queries) UpdateLinearInstallationToken(ctx context.Context, arg UpdateLinearInstallationTokenParams) (LinearInstallation, error) {
	row := q.db.QueryRow(ctx, updateLinearInstallationToken,
		arg.OrganizationID,
		arg.AccessTokenEncrypted,
		arg.RefreshTokenEncrypted,
		arg.ExpiresAt,
	)
	var i LinearInstallation
	err := row.Scan(
		&i.OrganizationID,
		&i.AppUserID,
		&i.AccessTokenEncrypted,
		&i.RefreshTokenEncrypted,
		&i.ExpiresAt,
		&i.ConnectedByUserID,
		&i.CreatedAt,
		&i.UpdatedAt,
	)
	return i, err
}

const upsertLinearInstallation = `-- name: UpsertLinearInstallation :one

INSERT INTO linear_installations (
    organization_id, app_user_id, access_token_encrypted,
    refresh_token_encrypted, expires_at, connected_by_user_id
) VALUES ($1, $2, $3, $4, $5, $6)
ON CONFLICT (organization_id) DO UPDATE SET
    app_user_id              = excluded.app_user_id,
    access_token_encrypted   = excluded.access_token_encrypted,
    refresh_token_encrypted  = excluded.refresh_token_encrypted,
    expires_at               = excluded.expires_at,
    connected_by_user_id     = excluded.connected_by_user_id,
    updated_at               = now()
RETURNING organization_id, app_user_id, access_token_encrypted, refresh_token_encrypted, expires_at, connected_by_user_id, created_at, updated_at
`

type UpsertLinearInstallationParams struct {
	OrganizationID        string             `json:"organization_id"`
	AppUserID             string             `json:"app_user_id"`
	AccessTokenEncrypted  []byte             `json:"access_token_encrypted"`
	RefreshTokenEncrypted []byte             `json:"refresh_token_encrypted"`
	ExpiresAt             pgtype.Timestamptz `json:"expires_at"`
	ConnectedByUserID     pgtype.UUID        `json:"connected_by_user_id"`
}

// Queries backing LinearInstallationStore (Step 34, "Linear ingress",
// §8.10's own "OAuth" scope -- migrations/000029_linear_installations.
// up.sql's own doc comment has the full "why this table, why keyed by
// organization_id" writeup).
// Installing (or RE-installing/re-authorizing, e.g. after a scope change
// or manual revoke-and-reconnect) the SAME workspace simply replaces its
// token pair in place -- there is exactly one live installation per
// organization_id, never a history of past ones.
func (q *Queries) UpsertLinearInstallation(ctx context.Context, arg UpsertLinearInstallationParams) (LinearInstallation, error) {
	row := q.db.QueryRow(ctx, upsertLinearInstallation,
		arg.OrganizationID,
		arg.AppUserID,
		arg.AccessTokenEncrypted,
		arg.RefreshTokenEncrypted,
		arg.ExpiresAt,
		arg.ConnectedByUserID,
	)
	var i LinearInstallation
	err := row.Scan(
		&i.OrganizationID,
		&i.AppUserID,
		&i.AccessTokenEncrypted,
		&i.RefreshTokenEncrypted,
		&i.ExpiresAt,
		&i.ConnectedByUserID,
		&i.CreatedAt,
		&i.UpdatedAt,
	)
	return i, err
}
