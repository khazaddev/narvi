...
// versions:
//   sqlc v1.31.1
// source: image_cache_versions.sql
...
const deleteCacheVersions = `-- name: DeleteCacheVersions :exec
DELETE FROM image_cache_versions
WHERE cache_key = $1 AND version = ANY($2::bigint[])
`

type DeleteCacheVersionsParams struct {
...
func (q *Queries) DeleteCacheVersions(ctx context.Context, arg DeleteCacheVersionsParams) error {
	_, err := q.db.Exec(ctx, deleteCacheVersions, arg.CacheKey, arg.Versions)
	return err
}

const latestCacheVersion = `-- name: LatestCacheVersion :one
...
func (q *Queries) LatestCacheVersion(ctx context.Context, cacheKey string) (int64, error) {
	row := q.db.QueryRow(ctx, latestCacheVersion, cacheKey)
	var version int64
	err := row.Scan(&version)
	return version, err
}

const listCacheVersions = `-- name: ListCacheVersions :many
SELECT version FROM image_cache_versions
WHERE cache_key = $1
ORDER BY version DESC
`
...
func (q *Queries) ListCacheVersions(ctx context.Context, cacheKey string) ([]int64, error) {
	rows, err := q.db.Query(ctx, listCacheVersions, cacheKey)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const mintCacheVersion = `-- name: MintCacheVersion :one
...
func (q *Queries) MintCacheVersion(ctx context.Context, cacheKey string) (int64, error) {
	row := q.db.QueryRow(ctx, mintCacheVersion, cacheKey)
	var next_version int64
	err := row.Scan(&next_version)
	return next_version, err
}

const publishCacheVersion = `-- name: PublishCacheVersion :exec
INSERT INTO image_cache_versions (cache_key, version, fingerprint, created_at)
VALUES ($1, $2, $3, now())
`

type PublishCacheVersionParams struct {
	CacheKey    string `json:"cache_key"`
...
func (q *Queries) PublishCacheVersion(ctx context.Context, arg PublishCacheVersionParams) error {
	_, err := q.db.Exec(ctx, publishCacheVersion, arg.CacheKey, arg.Version, arg.Fingerprint)
	return err
}
