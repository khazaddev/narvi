...
func (q *Queries) ListSubTaskFinishEventsForTurn(ctx context.Context, arg ListSubTaskFinishEventsForTurnParams) ([]Event, error) {
	rows, err := q.db.Query(ctx, listSubTaskFinishEventsForTurn, arg.SessionID, arg.Gen, arg.DispatchedAt)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) ListSubTaskStartEventsForTurn(ctx context.Context, arg ListSubTaskStartEventsForTurnParams) ([]Event, error) {
	rows, err := q.db.Query(ctx, listSubTaskStartEventsForTurn, arg.SessionID, arg.Gen, arg.DispatchedAt)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
