...
func newLivenessAdapter(t *testing.T, fake *fakeOpenCodeServer) *Adapter {
	t.Helper()
	a := New(fake.URL(), livenessSSEInactivityTimeout, livenessReconnectInterval, livenessRequestTimeout)
	t.Cleanup(a.Close)
	return a
}
...
func TestWaitForTurn_ReconnectWinsRaceAgainstFallback(t *testing.T) {
	...
	a := New(fake.URL(), reconnectRaceSSEInactivityTimeout, reconnectRaceReconnectInterval, livenessRequestTimeout)
	t.Cleanup(a.Close)
