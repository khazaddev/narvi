...
type RepoSettings struct {
	...
	// see that field's own description for why this is a self-governed figure the
	// agent reads, not a server-enforced spend cap.
	...
	// dollar figure STATED to the reviewing agent's own prompt
	// (internal/domain/review's own subAgentOrchestrationInstructions), which
	// self-governs against it: §26.7 is a self-reported, best-effort check, not a
	// server-enforced gate (this control plane has no channel to intervene inside an
	// already-dispatched turn) -- reviewtriage.ShouldSkipOptionalPass, the pure
	// function this ceiling would feed a real enforcement path, is not yet called by
	// any production path. The same 'changes what an unattended review is TOLD,
	// admin-gated' reasoning every sibling toggle in this row already carries, worded
	// here to avoid overstating this as an active runtime gate.
...
// figure STATED to the reviewing agent's own prompt (internal/domain/review's own
// subAgentOrchestrationInstructions), which self-governs against it: §26.7 is a
// self-reported, best-effort check, not a server-enforced gate (this control plane
// has no channel to intervene inside an already-dispatched turn) --
// reviewtriage.ShouldSkipOptionalPass, the pure function this ceiling would feed a
// real enforcement path, is not yet called by any production path. The same
// 'changes what an unattended review is TOLD, admin-gated' reasoning every sibling
// toggle in this row already carries, worded here to avoid overstating this as an
// active runtime gate.
