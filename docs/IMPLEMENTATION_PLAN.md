...
Steps 66-69 (the review decision-dossier chantier) at the end of Phase 5; see that phase's own
...
| 57 ∥ | RWX provider + previews | RWX adapter, preview links at the latest PR commit | §8.9 |
| 58 ∥ | uploads | Object storage (S3-compatible) + `download_file` tool, failed-upload signal | §8.6 |
| 59 | models | Catalog, Codex/ChatGPT OAuth plugin, per-session/per-message reasoning effort; shadow-comparison tooling for review; **Gemini finding RESOLVED** (verified 2026-08-03 against the live pinned OpenCode 1.17.15 binary's own `GET /provider` catalog, bypassing rtk's output-truncation — lists a `google` provider (env `GOOGLE_API_KEY`/`GOOGLE_GENERATIVE_AI_API_KEY`/`GEMINI_API_KEY`) plus `google-vertex`/`google-vertex-anthropic`, with real models e.g. `gemini-3.5-flash` carrying `capabilities.toolcall: true`, a 1M-token context window and real cost data — so no new `AgentRuntime` adapter is needed for Gemini, only Step 53's credential injection plus a CI contract test, mirroring the existing pinned-binary contract-test discipline, verifying Gemini's actual tool-calling/streaming quality through OpenCode, since every existing contract test is Claude-backed and proves nothing transitively about a different provider's behavior) | §8.8, §25.2 |
...
because they restructure the review verdict (§8.2/Steps 45/47) into a decision dossier and build
...
| 79 | ui code review + release review | **Decision-dossier layout (§26): digest sections first ("What this PR does", architecture choices, stack risks, contested points), findings in a collapsed appendix**; editable risk-map, sentinels, findings (apply/rebut), history, server-side badge; handoff-readiness sentinel (Step 49) on scoped sessions; **dedicated release-review screen: manifest + trigger banner + composition findings (Step 50)**; false-positive pattern management (view/retire per repo, maintainer+, Step 63); rebuttal history with finding-identity linkage (Step 48); epistemic-check outcome surfaced as a subtle "Heads-up" indicator in the review view when `minor`/`strong` fired (Step 61) | decisions 7-14, §14.4, §15, §20, §21, §22, §26 |
...
- **Phase 5**: 53 (provider credential injection) is a blocking prerequisite for 54-56 —
  ...
  posting path new patterns get weighed against) and 39 (`Authorize`). 64 depends on 36
  ...
  own primitives, never a mechanism of its own. 66 (decision-dossier digest) depends on 45/47
