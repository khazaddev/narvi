...
[docs/design/mockups.html](design/mockups.html). This plan breaks the 8 phases (0–7) into **~74 ordered Steps**
...
the later insertion of Steps 57-60 at the end of Phase 5, see that phase's own renumbering note).
...
## Phase 5 — Code review & automations (16 Steps)
...
| 47 | server-side verdict | Verdict-posting tool + raw-comment blocking (scoped to review sessions), `review:*` label sync, formal-review gate; the posting tool's payload is the structured `domain/review` verdict (Step 45) — the agent calls it with typed fields, never free text parsed back out of markdown; `blockOnHighRisk` (admin, per-repo, strict-boolean) reuses the same formal-review submission path and carries no independent permission of its own; **auto-approval is criteria-driven, never label-triggered** — the server recomputes `Shippable` deterministically and checks it against CI-green/no-floor-raised/diff-size/sensitive-path criteria (the eligibility engine itself ships in Step 58); the existing `review: low risk` label **inverts** into a `review: needs-human` escape hatch that forces human review on a PR regardless of criteria — a deliberate change to this Step's original label-driven design, flagged for validation | §8.2, §5.2, §21 |
...
| Step | Title | Content | Ref. |
|---|---|---|---|
| 61 | config/data seeding | Scripts to seed automations, secrets, environments, settings, integrations; participants→users mapping (by GitHub id, default member) | §10-P6, §13.4 |
| 62 | cohort rollout | Feature-flagged cohort rollout of sessions, documented rollback | §10-P6 |
| 63 | ops | Dashboards/alerts (false failures, outbox lag, orphans, boot p95), runbooks from the resilience catalog (§9.3) | §5.3 |
| 64 | launch readiness | Production checklist, SLO alerts wired, on-call runbook | §10-P6 |

## Phase 7 — UI (10 Steps, visual spec = nine-view mockups)

| Step | Title | Content | Ref. |
|---|---|---|---|
| 65 | ui bootstrap | Vite+React+TanStack, `go:embed` + `narvi serve`, light/dark theme tokens from the mockups, front CI | §12.1 |
| 66 | ui data layer | TS client generated from `/contracts`, WS transport → event-log → reducer | §12.1 |
| 67 | ui sign-in | GitHub primary, SSO OIDC, identity auto-link panel, allowlist errors | §12.2-7, §13.1 |
| 68 | ui session: timeline | Sidebar (status chips + session source, My sessions/All filter, boot n/m), typed-event timeline incl. sub-task lane nesting, failure cards + Resume | decisions 1-4, 31, §7.1 |
| 69 | ui session: composer & rail | Composer (model/effort/plan mode, warm-on-type), sandbox rail (transitions, gen, fingerprint, boot phases, artifacts, cost incl. sub-task roll-up) | decisions 5-6, §7.1 |
| 70 | ui code review + release review | Editable risk-map, sentinels, findings (apply/rebut), history, server-side badge; handoff-readiness sentinel (Step 49) on scoped sessions; **dedicated release-review screen: manifest + trigger banner + composition findings (Step 50)**; false-positive pattern management (view/retire per repo, maintainer+, Step 59); rebuttal history with finding-identity linkage (Step 48); epistemic-check outcome surfaced as a subtle "Heads-up" indicator in the review view when `minor`/`strong` fired (Step 57) | decisions 7-14, §14.4, §15, §20, §21, §22 |
| 71 | ui plan mode + automations | Versioned plan + multi-channel approval bar; automations health/runs table | decisions 15-20 |
| 72 | ui settings + analytics | Environments/secrets/templates/members&access (roles, identities, audit); **path-scope + services editor for product Environments**; analytics (KPIs incl. false failures and decision latency, charts); review-risk analytics section (KPI tiles, trend chart, top-risk table) fed by Step 58's read model with explicit "not available yet" states; digest cadence/scope settings per repo; per-repo auto-merge toggle (off by default) with contradiction-rate calibration stats displayed next to it | decisions 21-30, §14.1, §14.2, §21 |
| 73 | ui decision inbox (home) | Home view: decision queue by section (merge/review/approval/attention), inline actions wired to the Step 56 API, assignment provenance, staleness, repo-only filter (the inbox is inherently user-scoped), median time-to-decision; the sessions list moves to a second tab | decisions 32-34, §16 |
| 74 | ui finalize | `make dist` single self-contained binary, screenshot review vs mockups, ship | §12.4 |
...
- **Phase 7** can start 65-66 during phase 6 (backend and contracts are frozen by then).
