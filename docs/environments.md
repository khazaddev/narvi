...
Because of this, **`setup.sh` reruns on most warm boots** — not just on a cold/fresh boot the way it used to, though the two mechanisms in the next section avoid or shorten many of those reruns. Concretely (`internal/domain/sandboxboot.EvaluateHook`, §19.4, plus the graduated ladder §19.6 layers on top of it):

- On a `repo_image` boot, `setup.sh` is skipped **entirely** in exactly two cases: (a) the checked-out SHA still exactly matches the image's own baked `built_repo_shas` entry for that repo (`workspaceMoved == false`, `/narvi/image-manifest.json`) — the original, §19.4-only rule; or (b) `workspaceMoved == true`, but the tier-1 dependency-manifest-digest skip below (§19.6) proves the dependency surface is unchanged AND that `setup.sh` itself is unchanged. Case (b) **preempts the delta-script tier** described further below — when the digest tier fires, `sync.sh` is never even considered.
- In every other case, `setup.sh` reruns (in full, or via the optional delta script `sync.sh` when tier 2 below is eligible), **non-fatally**: a failed rerun is logged as a warning and the boot continues, exactly per the "never block a spawn" invariant (§10 Phase 2) — a moved workspace proves nothing about whether dependencies actually changed, so it can never justify failing the boot outright. A **full** `setup.sh` rerun (not `sync.sh`) that fails on its first attempt is retried **exactly once** before that warning is logged (§19.6: "retry the install on transient failure, then warn") — this exists for availability, not correctness: a package registry's brief hiccup should not degrade a warm boot the same way a genuine, repeatable failure does. If the retry also fails, the outcome is identical to today's single-attempt behavior — a warning, never a boot failure.

A `setup.sh` written under the old assumption ("I run exactly once, at image build, never again") can break in exactly this new situation: appending to a config file on every run, cloning a vendor directory it assumes doesn't already exist, seeding a database without an existence check, or simply exiting non-zero when it detects one of its own prior side effects. Under the old contract this was invisible because `setup.sh` genuinely never ran twice against the same image. Under the current contract it reruns on most warm boots, non-fatally — so a non-idempotent `setup.sh` degrades every warm boot to a silently-incomplete one (the workspace is left exactly as warm-cache-broken as the failed rerun left it, with only a log warning, never a boot failure) rather than failing loudly. This is the exact "non-idempotent-setup boot" scenario the resilience suite (§9.3) exists to bound — write your `setup.sh` to be re-runnable, and this failure mode never triggers.
...
Every shared image's own `/narvi/image-manifest.json` now also carries a `dependency_manifest_digests` map, alongside `built_repo_shas` — one digest per repo. See `docs/TECHNICAL_PLAN.md` §19.1 item 4 for the exact algorithm (this is the canonical, byte-for-byte specification an external build-time producer must match); in short: the repo's own working tree is walked recursively — skipping `.git`, `node_modules`, `vendor`, `bower_components`, `dist`, `build`, `target`, `.venv`/`venv`, and `__pycache__` at any depth — every file whose basename is one of the known dependency-lockfile names (`package-lock.json`, `pnpm-lock.yaml`, `yarn.lock`, `requirements.txt`, `Pipfile.lock`, `poetry.lock`, `go.sum`, `Cargo.lock`, `Gemfile.lock`, `composer.lock`) is hashed individually, and those per-file hashes are folded — each one keyed by its own **repo-relative path**, not just its basename, in sorted order — into one digest per repo. Folding the path means adding, removing, *or relocating* a manifest changes the digest, even when every remaining byte stays identical.

At boot, sandbox-agent recomputes the same digest against the checked-out tree. `setup.sh` is skipped entirely only when **both** of the following hold: the recomputed digest matches the baked value, **and** `setup.sh` itself is byte-identical between the image's own built SHA and the current checkout (the same check tier 2 below uses) — a digest match alone proves only that the dependency-manifest surface is unchanged, never that `setup.sh`'s own non-package-manager work (`apt-get install`, codegen, seeding local state, …) is a no-op, so it can never license the skip by itself.

This tier needs nothing from a repo author: it is fully automatic. Three cases fall through to the next tier, never a silent skip: an unreadable, missing, or unrecognized digest (an older image built before this tier existed, for example); a repo whose checked-out tree has **zero** recognized manifests anywhere (an ecosystem this tier doesn't recognize — Maven/Gradle, bun, uv, deno, mix, Swift, pubspec, .NET, … — a scan that finds no evidence is never treated as a match); and a **scoped** Environment's own boot (§19.7) — sparse-checkout can remove manifests from the working tree that the (always-unscoped, §19.1) baked digest accounted for, so a scoped boot never trusts this tier at all, regardless of what its own truncated tree happens to contain.
