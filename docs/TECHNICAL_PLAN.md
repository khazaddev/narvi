...
- **Two-phase terminalization**: a watchdog never writes `failed` directly. It writes `suspect` and arms `terminal_grace` (default 60s). Any liveness signal during grace returns to previous state. A genuinely late success (e.g. `execution_complete` arriving after terminalization) **reconciles**: turn marked completed, session status re-derived, automation run counters corrected. **Correction to an earlier overclaim**: this narrows the false-failure window, it does not eliminate it. The reconciliation above is scoped to a late signal that still finds the turn `Processing` — true whenever it arrives inside `turn_deadline`'s own separate, much longer budget (60 min vs. `terminal_grace`'s 60s, `platform/timeouts.go`), which is the ordinary case. What genuinely IS made unrepresentable is the CONTRADICTION class — a delivered artifact and a failure message both standing for the same turn at once — since a turn still `Processing` can only ever resolve to one clean terminal write, never a conflicting second one. The residual is real and stays a known limitation: a real `execution_complete` arriving after the turn has ALREADY gone terminal via its own `turn_deadline` timeout (a synthetic `execution_complete` already recorded, the turn already `failed`) is logged and discarded, not reconciled — the user is told the turn failed while the work actually completed, and that completion is then dropped on the floor. Forward-looking note: once a `failed` transition's own outbox notification (§5.1) is actually dispatched to the user, closing this residual requires late-success reconciliation to enqueue a CORRECTIVE follow-up notification through that same outbox — without it, the contradiction class this design otherwise prevents re-enters through exactly that door.
...
This event carries no comment body and no commenting actor at all, so the existing mention-detection step (doc.go's step 5, "detect whether the comment body mentions the bot") does not apply here — routing is by `(repo, PR)` identity alone, never by text.
...
4. Otherwise (§24.6's budget check, below) enqueues a new turn on the review session — the SAME `httpapi.CreateTurnForBot` path Step 46's own manual re-trigger already uses, so this is a second way to reach an existing dispatch primitive, never a second dispatch mechanism.
