...
The shipped design: a persistent, provider-backed cache volume mounted into the build sandbox at a package-manager-agnostic set of well-known cache paths (`domain/imagebuild.WellKnownCachePaths` — npm's `_cacache`, pip's HTTP cache, Go's module and build caches, Cargo's registry, and siblings for Yarn/pnpm/Composer/Bundler/Maven/Gradle), keyed on `Base` + `RuntimeVersion` **only** — never on repo content, since a cache that is not shared across repo sets recreates the very cold-start it exists to remove — with writes committed to the persistent volume only once the build that produced them has itself succeeded, so a failed build can never poison it. This landed as the promised `SandboxProvider` port extension: `ports.ImageSpec` gained an optional `CacheMount{Key, Paths}` field, computed by `app/imagebuild.Builder` via the new `domain/imagebuild.CacheVolumeKey(base, runtimeVersion)` (deliberately excluding repos, so every fingerprint sharing a `Base`/`RuntimeVersion` pair resolves to the identical volume regardless of repo set) and `WellKnownCachePaths`. It shipped as its own PR under Step 43, never a rider on the other two — it shares their theme, not their code: those reduce work at boot inside sandbox-agent, this reduces work at build behind a port. It was sequenced last of the three for size, not held behind a measurement (§19.9 records why the measurement gate an earlier draft put here did not survive this section's own argument); build-duration/failure-rate instrumentation shipped alongside it, ungated, to size the win and catch a regression (`app/imagebuild`'s own `image_build_duration_seconds` histogram and `image_build_attempt_total` counter, tagged by phase and outcome).

**Concurrency: decided as no lock, not left as an open question.** The two constraints an earlier draft of this paragraph left for "whoever schedules it" are now both resolved. Per-provider semantics differing is handled structurally: `CacheMount` carries only a plain key and path list, no lock handle and nothing provider-specific, and the field is purely advisory — `BuildImage`'s signature carries no cache-specific error, so an adapter (or its backing build infrastructure) that cannot safely mount the volume at all, for any reason, silently declines it and performs an ordinary cold build instead; a caller can never distinguish "the cache was used" from "the cache was declined" from the return value alone. The concurrency story itself is: **no lock, deliberately.** Every well-known cache path is a package manager's own *content-addressed* cache — the filename under `_cacache`, pip's HTTP cache, or `GOMODCACHE`/`GOCACHE` IS the content hash of what it holds — so two concurrent builds that happen to fetch the identical package version write identical bytes to the identical path. The "conflict" a lock would prevent is idempotent by construction: at worst a redundant download, never a corrupted cache. A lock would instead serialize §19.2's refresh pump — this design's own steady state, since it makes concurrent builds sharing one `Base`/`RuntimeVersion` cache volume routine, not rare — purely to guard against a collision that cannot corrupt anything. Two obligations follow from relying on that property instead of a lock, both stated in `ports.CacheMount`'s own doc comment as the port's documented contract rather than left implicit: every writer into one of these paths must rely on atomic rename semantics (write to a temp path in the same directory, then rename into the file's own final content-addressed name — every ecosystem named above already does this internally; no code in this repository performs such a write itself, since the mounting and the dependency install both happen inside the provider's build infrastructure), and writes must be committed to the persistent volume only after the build that produced them succeeds. `internal/adapters/outbound/modal`'s own `BuildImage` is the one adapter that exists today: it requests the cache mount on every build and, the instant its own wire protocol reports cache trouble (corrupted, locked, or unavailable), transparently retries once with the mount dropped — an ordinary cold build, indistinguishable on the wire from a request that never asked for a cache at all. RWX remains entirely unaffected (`Capabilities().ImageBuilds` is false; its own content-addressed layer cache already gives it this effect natively, §4.1.1).
