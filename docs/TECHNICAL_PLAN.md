...
**Build-time dependency cache: a named gap, designed here, currently unscheduled.** Item 2 above says `setup.sh`'s real payload is the warm dependency caches, and §19.4's rerun contract leans on the same property from the other end ("`npm ci`/`pip install`/`cargo build` against a warm cache is seconds, not minutes"). Both statements are about caches *inside an already-built image*. Neither says anything about the build itself — and today nothing carries a package cache *between* builds: every build starts from `Base` and re-downloads every dependency over the network. Two consequences follow. A build's duration, and more importantly its *failure probability*, scale with a repo's total dependency count rather than with whatever actually changed. And because the unit of caching is the whole image, a build that dies on one transient registry error discards all of its work, including the parts that had already succeeded. §19.2's refresh pump turns this from a per-Environment annoyance into a steady-state cost, since it rebuilds every `ready` image whenever its repos' tips move.

The design, when it is scheduled: a persistent, provider-backed cache volume mounted into the build sandbox at a package-manager-agnostic set of well-known cache paths, keyed on `Base` + `RuntimeVersion` **only** — never on repo content, since a cache that is not shared across repo sets recreates the very cold-start it exists to remove — with writes committed only after a successful build, so a failed build can never poison it. This needs a `SandboxProvider` port extension (`BuildImage` gaining a cache-mount concept) and therefore a Step of its own; it is deliberately not folded into an existing one. Two constraints for whoever schedules it: concurrent builds sharing one cache volume need an explicit concurrency story, and per-provider semantics differ enough that the port must not assume one; and the cache must behave as a pure accelerator — a corrupted, locked, or unavailable cache degrades to exactly today's cold build, never fails one.
...
- **Step 43** — graduated setup-rerun ladder (§19.6): scheduled once Step 42's rerun-duration telemetry shows full reruns materially eroding warm-boot latency, not shipped speculatively alongside it. Both §19.6 tiers ship together when it does: the dependency-manifest-digest skip (which needs §19.1's manifest to bake that digest) and the delta script.
- **Unscheduled, needs a Step of its own** — the build-time dependency cache (§19.1's own named gap): a persistent provider-backed cache volume mounted into the build sandbox, requiring a `SandboxProvider` port extension. Independent of Steps 40-43 and of every Phase 5-7 Step; schedule it whenever build failure rate or build duration justifies it, since neither is measured today.
