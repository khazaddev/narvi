...
**Build-time dependency cache: a named gap, designed here, shipped as Step 43's third PR (§19.9).** Item 2 above says `setup.sh`'s real payload is the warm dependency caches, and §19.4's rerun contract leans on the same property from the other end ("`npm ci`/`pip install`/`cargo build` against a warm cache is seconds, not minutes"). Both statements are about caches *inside an already-built image*. Neither says anything about the build itself — and before this PR nothing carried a package cache *between* builds: every build started from `Base` and re-downloaded every dependency over the network. Two consequences followed. A build's duration, and more importantly its *failure probability*, scaled with a repo's total dependency count rather than with whatever actually changed. And because the unit of caching was the whole image, a build that died on one transient registry error discarded all of its work, including the parts that had already succeeded. §19.2's refresh pump turns this from a per-Environment annoyance into a steady-state cost, since it rebuilds every `ready` image whenever its repos' tips move.

The shipped design: a persistent, provider-backed cache volume mounted into the build sandbox at a package-manager-agnostic set of well-known cache paths (`domain/imagebuild.WellKnownCachePaths()` — npm's `_cacache`, pip's HTTP cache, Go's module and build caches, Cargo's registry, and siblings for Yarn/pnpm/Composer/Bundler/Maven/Gradle), keyed on `Base` + `RuntimeVersion` + an explicit rotation epoch — never on repo content, since a cache that is not shared across repo sets recreates the very cold-start it exists to remove — mounted **read-only for the entire duration of the build**, with exactly **one write-back after the build has itself succeeded**, so a failed build can never poison it. This landed as the promised `SandboxProvider` port extension: `ports.ImageSpec` gained an optional `CacheMount{Key, Paths}` field, computed by `app/imagebuild.Builder` via the new `domain/imagebuild.CacheVolumeKey(base, runtimeVersion, epoch)` (deliberately excluding repos, so every fingerprint sharing a `Base`/`RuntimeVersion`/epoch triple resolves to the identical volume regardless of repo set) and `WellKnownCachePaths()`. It shipped as its own PR under Step 43, never a rider on the other two — it shares their theme, not their code: those reduce work at boot inside sandbox-agent, this reduces work at build behind a port. It was sequenced last of the three for size, not held behind a measurement (§19.9 records why the measurement gate an earlier draft put here did not survive this section's own argument); build-duration/failure-rate instrumentation shipped alongside it, ungated, to size the win and catch a regression (`app/imagebuild`'s own `image_build_duration_seconds` histogram and `image_build_attempt_total` counter, tagged by phase and outcome).

**Concurrency: decided as no lock — but the justification was wrong once, and this is the corrected version.** The very first PR that implemented this design mounted the volume read-write and justified skipping a lock on the grounds that every well-known cache path is a package manager's own "content-addressed" cache — the filename under `_cacache`, pip's HTTP cache, or `GOMODCACHE`/`GOCACHE` supposedly *IS* the content hash of what it holds, so two concurrent builds that happen to fetch the identical package version would write identical bytes to the identical path, and the "conflict" a lock would prevent would be idempotent by construction. **That premise was checked against each package manager's own real, documented on-disk layout and found false for nearly every path mounted here.** `$GOMODCACHE` ships its OWN lock file (`cache/lock`), a mutable `@v/list` version listing, and per-module `.lock` files — Go's own authors lock that cache precisely because concurrent access is not safe without coordination. Gradle's `caches/modules-2` is binary metadata mutated in place, guarded by `modules-2.lock`/`journal-1.lock` living INSIDE the mounted directory. Cargo guards its registry with `.package-cache`, which the original design did not even mount. npm's `_cacache/index-v5` entries are keyed by the hash of the *request URL*, not the response content, and are written by *append*, not atomic rename. pip's cache key is the SHA-224 of the request URL, and its `wheels/` subtree holds locally built artifacts. Composer, Bundler, and Yarn are similar. Worse than a missing lock: mounting a directory that carries a tool's own host-local lock file across sandboxes running on *different hosts* hands that tool a **false** sense of mutual exclusion — an advisory lock is invisible across hosts by construction. This is exactly the class of false premise §17.6/§20 already exist to catch on the review side; here it shipped once before being caught, which is precisely why it is corrected in the plan text rather than only in the code.

The no-lock decision itself still stands — its justification does not. The fix is not a smarter lock, it is mounting the volume **read-only for the duration of the build** so there is nothing left for a lock to guard: nothing writes into the shared, persistent copy while a build can observe it, so there is no interleaving to reason about, and a tool's own host-local advisory lock becomes irrelevant. Exactly one write-back happens, merging whatever a build newly produced, and only after that build has itself succeeded — never before, and never for a failed build, preserving the original "writes committed only after success" invariant unchanged. This is also, finally, the literal implementation of a sentence §19.1 asserted from its very first draft but never actually built: "writes committed only after a successful build" existed only in prose, expressed nowhere in the port shape or the wire request, until this correction made it the port's actual mechanism. A package manager that needs to write during the build (nearly all of them do, at least for a newly-resolved dependency) is given a private, per-build writable layer at these same logical paths — mechanism owned by the adapter/build service, external to this repository, exactly as the mount itself and the dependency install already are; this codebase's own Go code performs no such write itself. Per-provider semantics differing is handled structurally, unchanged from the original design: `CacheMount` carries only a plain key and path list, no lock handle and nothing provider-specific, and the field is purely advisory — `BuildImage`'s signature carries no cache-specific error, so an adapter (or its backing build infrastructure) that cannot safely mount the volume at all, for any reason, silently declines it and performs an ordinary cold build instead; a caller can never distinguish "the cache was used" from "the cache was declined" from the return value alone. `internal/adapters/outbound/modal`'s own `BuildImage` is the one adapter that exists today: it requests the cache mount on every build and, the instant its own wire protocol reports cache trouble — now broadened beyond three structured error codes to also cover a transport-level hang and an unparseable response (a degraded cache-volume subsystem typically hangs past `ProviderHTTPClientTimeout` rather than returning a clean error) — transparently retries once with the mount dropped — an ordinary cold build, indistinguishable on the wire from a request that never asked for a cache at all. RWX remains entirely unaffected (`Capabilities().ImageBuilds` is false; its own content-addressed layer cache already gives it this effect natively, §4.1.1).

**Rotation epoch: the escape hatch a pure `(Base, RuntimeVersion)` key did not have.** Before this correction, the only way to force a fresh cache volume for a pair that had become unusable — corrupted beyond a build service's own integrity check, or simply grown past a size bound with no eviction (see below) — was to bump `RuntimeVersion`, which is *also* a `Fingerprint` input, so that same bump invalidates every shared *image* fleet-wide too (this section's own "simultaneous-invalidation cliff," above), forcing every Environment's first post-bump build into the same window purely to escape a cache-volume problem unrelated to the images themselves. `domain/imagebuild.CacheVolumeKey` now takes a third input, `epoch` (`platform.Config.CacheVolumeEpoch`, an operator-controlled value, never hard-coded, never a `Fingerprint` input): bumping it mints a brand-new, empty cache volume for every `Base`/`RuntimeVersion` pair at once, while every already-`ready` image stays exactly as valid and servable as before — the cache is purely an accelerator, so rotating it can never affect correctness, only how warm the next build's cache starts out.

**Size is unbounded — a named gap, not a solved one.** Nothing in this design bounds, evicts, or expires anything in a cache volume, and one volume is shared fleet-wide — so, left alone, it grows until the provider's own storage quota is hit and an in-sandbox dependency install starts failing with `ENOSPC`. No enforcement ships in this Step; the rotation epoch above is today's only escape hatch (mint a fresh, empty volume — the old one is simply abandoned, never explicitly deleted). Real enforcement — a hard byte cap, LRU eviction inside the volume, or both — is deliberately deferred to a later Step, named here explicitly rather than left implicit, mirroring §19.2's own "newly urgent, still deferred" image-GC posture immediately above.
...
- CI green at head.
- No floor raised: neither the coverage floor nor the premise floor (§8.2/Step 45's `max(rank)` composition) is above its baseline. Note this one is **subsumed by `Shippable == auto` above and needs no separate check**: `Shippable` is the `max` of the baseline and both floors' ranks, with `auto` at rank 0, so `auto` is only reachable when all three inputs are already at rank 0. It stays listed here because it is a criterion of the policy, not because it is a second code path.
...
**Cross-step dependency, stated because it is not obvious:** turning changed paths into the `BlastRadius` tags this criterion matches on requires a path→tag classifier, and the design for one lives in §26.3 (Step 68) — six Steps later. Step 62 therefore ships its own deliberately over-inclusive interim classifier so the criterion is genuinely enforceable when the engine first exists; Step 68's per-repo-configurable globs supersede it. Without that, this criterion cannot be honestly implemented at Step 62 at all, and the temptation is to fall back on what the model reported — which is exactly the failure above.
