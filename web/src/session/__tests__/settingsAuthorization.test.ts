...
// updateMemberRole/rotateCloudIdentitySigningKey (api/endpoints.ts) are
// unconditional network calls with no client-side authorization logic of
// their own. The real gate is server-side (authz.ActionManageRepoSecrets/
// ActionManageMembers/ActionManageCloudIdentityKeys respectively,
// internal/domain/authz), already proven directly against a real
// Postgres instance by this package's own Go integration tests
// (sandboxsecrets_integration_test.go, members_integration_test.go,
// cloudidentitykeys_integration_test.go). This file proves the client
// half: the request really goes out over the wire, with the real
// method, and a 403 surfaces as a genuine, typed error rather than being
// silently swallowed or treated as success.
import { afterEach, describe, expect, it, vi } from 'vitest'

import { createSandboxSecret, rotateCloudIdentitySigningKey, updateMemberRole } from '../../api/endpoints'
...
describe('rotateCloudIdentitySigningKey -- the client always sends the real, destructive-adjacent request', () => {
  it('calls POST /api/cloud-identity/signing-keys/rotate unconditionally -- the confirm-before-rotate UI (EnvironmentsPanel.tsx) is a client-side nicety, never the real gate', async () => {
